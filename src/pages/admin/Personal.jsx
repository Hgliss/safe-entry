...
const handleExportPDF = () => {
  ...
  try {
    ...
    doc.setFontSize(16);
    doc.text("Reporte de Personal - SafeEntry", 40, 50);
    doc.setFontSize(10);
    ...
    doc.save("personal_safeentry.pdf");
  } catch (error) {
...
const handleExportExcel = () => {
  try {
    ...
    XLSX.writeFile(wb, "personal_safeentry.xlsx");
  } catch (error) {
    ...
    <div className="text-gray-900">
      ...
      <div className="mb-6 flex flex-wrap gap-2 items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-semibold text-blue-900">Personal</h1>
        </div>
        <div className="flex gap-2">
          ...
          >
            <Plus className="h-4 w-4" /> Nuevo Personal / Maestro
          </button>
        </div>
      </div>
      {/* Buscador */}
      ...
      {isOpen && (
        <Modal title={editing ? "Editar Personal / Maestro" : "Nuevo Personal / Maestro"} onClose={() => setIsOpen(false)}>
...
function mapToEntity(f) {
  return { ...f, rol_id: ROLE_TEACHER_ID };
}
